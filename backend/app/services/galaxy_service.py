...
class GalaxyService:
    ...
    def __init__(self, cache: CacheService):
        ...
        if not self.settings.GALAXY_API_KEY:
            ...
            logger.info(
                f"Galaxy service initialized with BioBLEND for URL: {self.settings.GALAXY_API_URL}"
            )
    ...
    async def get_job_status(self, job_id: str) -> GalaxyJobStatus:
        ...
        try:
            ...
            logger.info(
                f"BioBLEND Job {job_id} current state: {job_data['state']}, complete: {status.is_complete}, successful: {status.is_successful}"
            )
        ...
        try:
            ...
            for history in histories:
                if history.get("name") == shared_history_name:
                    ...
                    logger.info(
                        f"Using existing shared history: {history_id} ({shared_history_name})"
                    )
